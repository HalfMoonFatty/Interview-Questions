...
class Solution(object):
    def getSkyline(self, buildings):
        ...
        for p in points:   # sweep line
            if p[2] == 's': 
                heapq.heappush(maxHeap, -buildings[p[1]][2])
            else: 
                maxHeap.remove(-buildings[p[1]][2])
            ...
            if len(skyline) == 0 or skyline[-1][0] != p[0]:
                skyline.append([p[0], height])
            # in case start point duplicate
            elif p[2] == 's':
                skyline[-1][1] = max(skyline[-1][1], height)
            else:
                skyline[-1][1] = min(skyline[-1][1], height)
