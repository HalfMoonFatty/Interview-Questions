...
class Solution(object):
    def findTilt(self, root):
        ...
        :rtype: int
        """
        def subTreeSum(root,treeTilt):
            if not root: return 0
            lSum = subTreeSum(root.left,treeTilt)
            rSum = subTreeSum(root.right,treeTilt)
            nSum = root.val + lSum + rSum
            treeTilt[0] += abs(lSum-rSum)
            return nSum
            
        treeTilt = [0]
        subTreeSum(root,treeTilt)
        return treeTilt[0]
