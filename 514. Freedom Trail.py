...
Initially, the 1st character of the ring is aligned at 12:00 direction. You need to spell all the characters in the string key one by one by 
rotating the ring clockwise or anticlockwise to make each char of the string key aligned at 12 direction and then by pressing the center button.
    ...
    def findRotateSteps(self, ring, key):
        ...
        for c in key:
            ...
            for i in rlist[c]:
                ndp[i] = sys.maxint
                for j in dp:
                    ndp[i] = min(ndp[i], dp[j] + min(abs(i - j), rsize - abs(i - j)))
            dp = ndp
