...
class Solution(object):
    def findRedundantConnection(self, edges):
        ...
        def find(x, unionSet):
            if x == unionSet[x]:
                return x
            unionSet[x] = find(unionSet[x], unionSet)
            return unionSet[x]
        
        def union(root1, root2, unionSet):
            root1 = find(root1, unionSet)
            root2 = find(root2, unionSet)
            unionSet[root2] = root1
            return 
    
        unionSet = {i:i for i in range(1,len(edges)+1)}
        for pair in edges:
            root1, root2 = find(pair[0], unionSet), find(pair[1], unionSet)
            if root1 != root2:
                union(root1, root2, unionSet)
            else:
                return pair
