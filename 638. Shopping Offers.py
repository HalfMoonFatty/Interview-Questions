...
'''
Solution: 记忆化搜索（Search + Memoization）
...
class Solution(object):
    def shoppingOffers(self, price, special, needs):
        ...
        def minCosts(needs):
            if needs in cache: return cache[needs]
            cache[needs] = sum(n*p for n, p in zip(needs, price))    # init cache[needs]
            for sp in special:
                nneeds = tuple(n - s for n, s in zip(needs, sp))     # try every special offers
                if min(nneeds) < 0: continue
                cache[needs] = min(cache[needs], minCosts(nneeds)+sp[-1])
            return cache[needs]
        
        cache = {}
        return minCosts(tuple(needs))
