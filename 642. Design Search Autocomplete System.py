...
class AutocompleteSystem(object):
    ...
    def input(self, c):
        ...
        result = []
        if c == '#':
            self.addSentence(self.buffer, 1)
            self.buffer = ''
            self.cursor = self.root
        else:
            self.buffer += c
            ...
            if self.cursor: 
                result = self.lookup(self.buffer, self.cursor)
    ...
    def lookup(self, s, node):
        if node.times > 0: 
            self.result.append([s, node.times])
        for char in string.ascii_lowercase + ' ':
            if node.children.has_key(char):
                self.lookup(s+char, node.children[char])
        return self.result
